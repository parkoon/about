        ...
        lookBack: [
          {
            title: "App Router 도입",
            descriptions: [
              "프로젝트를 세팅할 때 App Router가 나온지 얼마 안됐지만, Pages Router를 사용하는건 레거시라는 생각에 App Router로 프로젝트 기반 작업을 시작했다.",
          ...
          },
          {
            ...
            descriptions: [
              ...
              "이러한 이유로 cookie 기반으로 다국어를 처리할 수 있는 라이브러리가 필요했다. 다국어 처리의 핵심은 설정된 언어를 기준으로 파일을 읽고, 번역 키 값을 이용해 텍스트를 가져오는 정도라 라이브러리를 직접 만드는데 큰 어려움음 없었다. 텍스트 내부에 변수 처리를 정규 표현식으로 처리해야 하는 부분이 조금 까다로웠지만 검색을 통해서 충분히 해결할 수 있는 정도였다.",
              "언어가 설정되어 있지 않은 경우, next middleware 에서 기본 언어 쿠키에 세팅해주고 redirect를 해주고 있었는데 쿠키를 가져갈 수 없는 환경에서는 무한 redirect가 발생했다. (실제로, 웹 크롤러가 og image 처리를 위해 페이지에 접근했을 때 문제가 발생했다.) ",
